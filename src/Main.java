...
public class Main extends PApplet {
    ...
    void updateWeapons() {
        if (s1.getCurrentWeapon() instanceof Scythe) {
            ((Scythe)s1.getCurrentWeapon()).update();
        } else if (s1.getCurrentWeapon() instanceof Mace) {
            ((Mace)s1.getCurrentWeapon()).update();
        }
    }

    void displayWeapons() {
        if (s1.getCurrentWeapon() instanceof Scythe) {
            ((Scythe)s1.getCurrentWeapon()).display();
        } else if (s1.getCurrentWeapon() instanceof Mace) {
            ((Mace)s1.getCurrentWeapon()).display(s1.getX(), s1.getY());
        }
    }
    ...
    public void keyPressed() {
        if (key == '1') s1.switchWeapon(0);
        if (key == '2') s1.switchWeapon(1);
        if (key == '3') s1.switchWeapon(2);
        if (key == '4') s1.switchWeapon(3);
        if (key == '5') s1.switchWeapon(4);
        if (key == jumpKey) jumpKeyHeld = true;
        if (key == leftKey) leftKeyHeld = true;
        if (key == rightKey) rightKeyHeld = true;
        if (key == dashKey) s1.dash();
    }

    public void keyReleased() {
        if (key == jumpKey) jumpKeyHeld = false;
        if (key == leftKey) leftKeyHeld = false;
        if (key == rightKey) rightKeyHeld = false;
    }

    void controlScheme() {
        if (jumpKeyHeld) s1.jump();
        if (leftKeyHeld) s1.walk(-3);
        if (rightKeyHeld) s1.walk(3);
    }
    ...
    class Sal {
        ...
        public void attack() {
            if (currentWeapon != null) currentWeapon.mainAttack(this, facing.equals("right") ? Direction.RIGHT : Direction.LEFT);
        }

        public void switchWeapon(int index) {
            if (index >= 0 && index < weaponSlots.length) currentWeapon = weaponSlots[index];
        }
